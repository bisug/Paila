...
| HomeFeed | loaded | C | Recommendation "Picked for your next visit" `+1` mark-visited button is ~24px tall (< 44px touch target) | Low | Fixed (responsiveness pass) — `px-3 min-w-[44px]`, see `audit/responsiveness.md` |
...
| Guides Index | loaded | C | "All"/"Saved" segmented toggle buttons are `py-1.5` (~28px), under 44px touch target | Low | Fixed (responsiveness pass) — `py-2.5 min-h-[44px]`, see `audit/responsiveness.md` |
...
## Summary
- Issues found: 14
- Fixed: 11 (9 in this pass + 2 resolved by the app-wide touch-target pass, see `audit/responsiveness.md`)
- Deferred: 3 (Guide Profile bookmark button, HomeFeed nested-interactive structural refactor, WeatherWidget error)
...
- **Guide Profile bookmark button (C/Low):** `h-9` matches sibling bookmark buttons app-wide; cosmetic.
